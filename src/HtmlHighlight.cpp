...
{
    bool highlighted = filetype().isEmpty();
    ...
    while (changeIndex < change.size()) {
        ...
        stream << flush;
        std::string input = plainText.constData();
        _sourceHighlight.getHighlighter()->highlightParagraph(input);
        std::string output = _sourceHighlight.getBuffer().str();
        changedLine.setHighlightText(QString::fromUtf8(output.c_str()));
        ...
        highlighted = highlighted || output != input;
